...
enrolment, a **Settings** tab exposing **every** option with search and inline
help, and a **Startup** tab that installs and removes the login entry. **Pause
locking** is always one click away.

The Settings tab is generated from the field registry in `config.py`, so it can
never drift out of step with the config — a test fails if a setting exists
without an entry. Advanced options are hidden behind a toggle but always
...
**There is no liveness detection, and a photograph of you will satisfy it.** A
2D anti-spoofing classifier was tried and removed: it was defeated by a photo on
a phone screen in the first real test, and a security control that does not work
is worse than none, because you would trust it. Catching that properly needs a
depth or infrared camera.
